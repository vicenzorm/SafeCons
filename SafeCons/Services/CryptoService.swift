...
protocol CryptoServiceProtocol {
    ...
    func generateIdentityColors(from name: String) -> [String]
}
...
final class CryptoService: CryptoServiceProtocol {
    ...
    func generateIdentityColors(from name: String) -> [String] {
        ...
        let hex1 = String(format: "#%02X%02X%02X", bytes[0], bytes[1], bytes[2])
        let hex2 = String(format: "#%02X%02X%02X", bytes[3], bytes[4], bytes[5])
